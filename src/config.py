...
config = {
    ...
    'supported_resolution': {
        'ratio': '16:9', #支持的游戏分辨率
        'min_size': (549, 945), #支持的最低游戏分辨率
        'resize_to': [(549, 945)], #可选, 如果非16:9自动缩放为 resize_to
    },
    'links': { # 关于里显示的链接, 可选
            'default': {
    ...
    'template_matching': { # 可选, 如使用OpenCV的模板匹配
        'coco_feature_json': os.path.join('assets', 'result2.json'), #coco格式标记, 需要png图片, 在debug模式运行后, 会对进行切图仅保留被标记部分以减少图片大小
    ...
    'onetime_tasks': [  # 用户点击触发的任务
        ...
        ["ok", "DiagnosisTask"],
        # ["src.tasks.MyNewTask", 'MyOneTimeTask'],
        ["src.tasks.Auto_bettle.Auto_ft", 'MyOneTimeTask'],
    ],
    'trigger_tasks':[ # 不断执行的触发式任务
        ["src.tasks.Auto_LianJin", "MyTriggerTask"],
    ],
    'custom_tabs': [
